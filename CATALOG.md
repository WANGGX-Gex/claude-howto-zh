...
> 所有 Claude Code 功能的快速参考指南：命令、代理（Agent）、技能（Skill）、插件（Plugin）和钩子（Hook）。

**导航**: [命令](#斜杠命令) | [权限模式](#权限模式) | [子代理](#子代理) | [技能](#技能) | [插件](#插件) | [MCP 服务器](#mcp-服务器) | [钩子](#钩子) | [记忆](#记忆文件) | [新功能](#新功能2026-年-3-月)
...
| 功能 | 内置 | 示例 | 总计 | 参考 |
|------|------|------|------|------|
| **斜杠命令（Slash Command）** | 55+ | 8 | 63+ | [01-slash-commands/](01-slash-commands/) |
| **子代理（Subagent）** | 6 | 10 | 16 | [04-subagents/](04-subagents/) |
...
| `/plugin` | 管理插件 | 安装/移除扩展 |
| `/plan` | 进入规划模式 | 复杂实现 |
| `/rewind` | 回退到检查点（Checkpoint） | 撤销更改，探索替代方案 |
| `/checkpoint` | 管理检查点 | 保存/恢复状态 |
| `/cost` | 显示令牌（Token）使用费用 | 监控开支 |
| `/context` | 显示上下文窗口（Context Window）使用量 | 管理对话长度 |
| `/export` | 导出对话 | 保存供参考 |
| `/extra-usage` | 配置额外使用限制 | 速率限制管理 |
| `/feedback` | 提交反馈或 Bug 报告 | 报告问题 |
| `/login` | 使用 Anthropic 进行认证 | 访问功能 |
| `/logout` | 退出登录 | 切换账户 |
| `/sandbox` | 切换沙盒（Sandbox）模式 | 安全命令执行 |
...
| 命令 | 描述 | 使用场景 | 作用域 | 安装 |
|------|------|---------|--------|------|
| `/optimize` | 分析代码进行优化 | 性能改进 | 项目 | `cp 01-slash-commands/optimize.md .claude/commands/` |
...
Claude Code 支持 6 种权限模式（Permission Mode），控制工具使用的授权方式。

| 模式 | 描述 | 使用场景 |
|------|------|---------|
| `default` | 每次工具调用都提示确认 | 标准交互使用 |
| `acceptEdits` | 自动接受文件编辑，其他操作需提示 | 受信任的编辑工作流 |
| `plan` | 仅允许只读工具，不可写入 | 规划和探索 |
| `auto` | 不提示即接受所有工具 | 完全自主操作（研究预览） |
...
| **Claude Code Guide** | 帮助和文档 | Read, Glob, Grep | Haiku 4.5 | 获取帮助、了解功能 |
...
| 代理 | 描述 | 使用场景 | 作用域 | 安装 |
|------|------|---------|--------|------|
| `code-reviewer` | 综合代码质量分析 | 代码审查会话 | 项目 | `cp 04-subagents/code-reviewer.md .claude/agents/` |
...
| 技能 | 描述 | 自动调用时机 | 作用域 | 安装 |
|------|------|-------------|--------|------|
| `code-review` | 综合代码审查 | "Review this code", "Check quality" | 项目 | `cp -r 03-skills/code-review .claude/skills/` |
...
| `refactor` | 系统化代码重构（Martin Fowler 方法） | "Refactor this", "Clean up code" | 用户 | `cp -r 03-skills/refactor ~/.claude/skills/` |
...
技能在 `SKILL.md` 中支持 YAML 前置元数据进行配置：
...
命令、代理、MCP 服务器和钩子的集合包。

### 示例插件

| 插件 | 描述 | 组件 | 使用场景 | 作用域 | 安装 |
|------|------|------|---------|--------|------|
...
> **作用域**: `项目` = 团队 (`.mcp.json`), `用户` = 个人 (`~/.claude.json`), `内置` = 预安装
...
| `InstructionsLoaded` | 指令已加载 | CLAUDE.md 或规则文件加载时 | 自定义指令处理 |
| `UserPromptSubmit` | 提示词处理前 | 用户发送消息 | 输入验证 |
...
| `Notification` | 通知发送 | Claude 发送通知 | 外部告警 |
| `SubagentStart` | 子代理启动 | 子代理任务开始 | 初始化子代理上下文 |
| `SubagentStop` | 子代理完成 | 子代理任务完成 | 链式操作 |
| `Stop` | Claude 完成回复 | 回复完成 | 清理、报告 |
| `StopFailure` | API 错误结束回合 | API 错误发生 | 错误恢复、日志 |
| `TeammateIdle` | 队友代理空闲 | 代理团队协调 | 分配工作 |
| `TaskCompleted` | 任务标记完成 | 任务完成 | 后续处理 |
| `TaskCreated` | 通过 TaskCreate 创建任务 | 新任务创建 | 任务跟踪、日志 |
| `ConfigChange` | 配置更新 | 设置修改 | 响应配置变更 |
| `CwdChanged` | 工作目录更改 | 目录切换 | 目录特定设置 |
| `FileChanged` | 监控的文件变更 | 文件修改 | 文件监控、重建 |
| `PreCompact` | 上下文压缩（Context Compaction）前 | 上下文压缩开始 | 状态保存 |
| `PostCompact` | 压缩完成后 | 压缩完成 | 后续操作 |
| `WorktreeCreate` | 工作树（Worktree）创建中 | Git 工作树创建 | 设置工作树环境 |
| `WorktreeRemove` | 工作树移除中 | Git 工作树移除 | 清理工作树资源 |
| `Elicitation` | MCP 服务器请求输入 | MCP 引导请求 | 输入验证 |
| `ElicitationResult` | 用户响应引导 | 用户回复 | 响应处理 |
| `SessionEnd` | 会话终止 | 会话结束 | 清理、保存状态 |
...
| `context-tracker.py` | 令牌使用跟踪 | Stop | 用户 | `cp 06-hooks/context-tracker.py ~/.claude/hooks/` |
...
| **本地** | `./CLAUDE.local.md` | 本地（git 忽略） | 机器特定覆盖（截至 2026 年 3 月未在官方文档中；可能为遗留功能） |
| **自动记忆** | 自动 | 会话 | 自动捕获的洞察和更正 |
...
| 功能 | 描述 | 使用方式 |
|------|------|---------|
| **远程控制** | 通过 API 远程控制 Claude Code 会话 | 使用远程控制 API 以编程方式发送提示词和接收响应 |
| **Web 会话** | 在基于浏览器的环境中运行 Claude Code | 通过 `claude web` 或 Anthropic Console 访问 |
| **桌面应用** | Claude Code 原生桌面应用程序 | 使用 `/desktop` 或从 Anthropic 网站下载 |
| **代理团队** | 协调多个代理处理相关任务 | 配置协作并共享上下文的队友代理 |
| **任务列表** | 后台任务管理和监控 | 使用 `/tasks` 查看和管理后台操作 |
| **提示词建议** | 上下文感知的命令建议 | 根据当前上下文自动显示建议 |
| **Git 工作树** | 并行开发的隔离 Git 工作树 | 使用工作树命令进行安全的并行分支工作 |
| **沙盒** | 安全的隔离执行环境 | 使用 `/sandbox` 切换；在受限环境中运行命令 |
| **MCP OAuth** | MCP 服务器的 OAuth 认证 | 在 MCP 服务器设置中配置 OAuth 凭据以进行安全访问 |
| **MCP 工具搜索** | 动态搜索和发现 MCP 工具 | 使用工具搜索查找跨连接服务器的可用 MCP 工具 |
| **定时任务** | 使用 `/loop` 和 cron 工具设置周期性任务 | 使用 `/loop 5m /command` 或 CronCreate 工具 |
| **Chrome 集成** | 使用无头 Chromium 的浏览器自动化 | 使用 `--chrome` 标志或 `/chrome` 命令 |
| **键盘自定义** | 自定义键位绑定，包括连击支持 | 使用 `/keybindings` 或编辑 `~/.claude/keybindings.json` |
| **Auto 模式** | 无权限提示的完全自主操作（研究预览） | 使用 `--mode auto` 或 `/permissions auto`；2026 年 3 月 |
| **频道** | 多频道通信（Telegram、Slack 等）（研究预览） | 配置频道插件；2026 年 3 月 |
| **语音听写** | 提示词的语音输入 | 使用麦克风图标或语音键绑定 |
| **代理钩子类型** | 生成子代理而非运行 Shell 命令的钩子 | 在钩子配置中设置 `"type": "agent"` |
| **提示词钩子类型** | 将提示词文本注入对话的钩子 | 在钩子配置中设置 `"type": "prompt"` |
...
| **插件 LSP 支持** | 通过插件的语言服务器协议集成 | 在 `plugin.json` 中配置 LSP 服务器以获取编辑器功能 |
| **管理插件** | 组织管理的预置配置（v2.1.83） | 由管理员通过管理策略配置；自动应用于所有用户 |
