...
graph LR
    ...
    C -->|功能| E["单次查询<br>可脚本化<br>可管道<br>JSON 输出"]
```
...
| `--effort` | 设置思考力度等级（low、medium、high、max） | `claude --effort high` |
...
| `--json-schema` | 获取符合模式的验证 JSON | | `claude -p --json-schema '{"type":"object"}' "query"` |
...
将 Claude 作为可编程 API 用于你的工具，配合 `jq` 解析。
...
| 模型 | ID | 上下文窗口 | 备注 |
|------|-----|-----------|------|
| Opus 4.6 | `claude-opus-4-6` | 100 万令牌 | 最强大，自适应思考力度等级 |
| Sonnet 4.6 | `claude-sonnet-4-6` | 100 万令牌 | 速度与能力平衡 |
| Haiku 4.5 | `claude-haiku-4-5` | 100 万令牌 | 最快，适合快速任务 |
...
提示词中的关键词 "ultrathink" 可激活深度推理。`max` 思考力度等级为 Opus 4.6 独有。
...
| `MAX_THINKING_TOKENS` | 设置扩展思考令牌预算 |
| `CLAUDE_CODE_EFFORT_LEVEL` | 设置思考力度等级（`low`/`medium`/`high`/`max`） |
...
| `CLAUDE_CODE_DISABLE_1M_CONTEXT` | 禁用 100 万令牌上下文窗口 |
...
| `MAX_MCP_OUTPUT_TOKENS` | MCP 工具输出的最大令牌数 |
...
- 使用 `--json-schema` 强制结构
- 在提示词中添加明确的 JSON 指令
- 使用 `--output-format json`（而不仅仅在提示词中要求 JSON）
...
- 用于自动化时谨慎使用 `--dangerously-skip-permissions`
...
- **[记忆指南](../02-memory/)** - 通过 CLAUDE.md 实现的持久化上下文
